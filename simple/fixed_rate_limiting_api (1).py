#!/usr/bin/env python3
"""
Complete Fixed API Server with Working Rate Limiting
All original functionality preserved with rate limiting issues resolved
"""
...
class FixedRateLimitConfig:
    """FIXED: Rate limit configuration with proper environment handling"""
        ...
        if environment == 'development':
            ...
            self.endpoint_limits = {
                ...
                '/api/v1/upload': {
                    ...
                    'user': RateLimit(10, 300, "User upload: 10 per 5 minutes"),
                },
                '/api/v1/posts': {
                    'ip': RateLimit(10, 300, "Posts: 10 per 5 minutes"),
                    'user': RateLimit(20, 300, "User posts: 20 per 5 minutes"),
                },
                '/api/v1/search/posts': {
                    'ip': RateLimit(10, 60, "Search: 10 per minute"),
                    'user': RateLimit(20, 60, "User search: 20 per minute"),
                },
            }
            
            self.default_limits = {
                'ip': RateLimit(30, 300, "Default IP: 30 per 5 minutes"),
                'user': RateLimit(100, 300, "Default user: 100 per 5 minutes"),
            }
        else:
            # Production limits
            self.endpoint_limits = {
    ...
    def get_limit_for_request(self, path: str, method: str, limit_type: str, user_role: str = 'user') -> RateLimit:
        ...
        # Final fallback
        return RateLimit(10, 60, "Fallback limit")
...
class FixedRateLimitMiddleware:
    ...
    def __init__(self, config: Optional[FixedRateLimitConfig] = None):
        self.limiter = FixedRateLimiter()
        self.config = config or FixedRateLimitConfig()
...
def get_rate_limiter():
    ...
    if _rate_limiter_instance is None:
        ...
        config = FixedRateLimitConfig(environment=environment)
...
async def init_app(app):
    ...
    try:
        ...
        environment = os.getenv('ENVIRONMENT', 'development')
        logging.info(f"Server starting in {environment} environment with rate limiting")
...
app.middlewares.append(rate_limit_middleware)      # Apply rate limiting - NOW PROPERLY WORKING
...
if __name__ == '__main__':
    ...
    if use_ssl:
        ...
        logging.info("🛡️  FEATURES: Rate Limiting, Authentication, CSRF Protection, File Upload, Search, Admin Panel")
        web.run_app(
            app,
            host=host,
            port=port,
        ...
        logging.info(f"   curl http://localhost:{port}/api/v1/csrf-token  # 3 requests/minute limit")
        logging.info(f"   curl -X POST http://localhost:{port}/api/v1/auth/login -H 'Content-Type: application/json' -d '{{\"email\":\"test@example.com\",\"password\":\"password\"}}'  # 5 requests/5min limit")
        logging.info(f"   curl http://localhost:{port}/api/v1/search/posts?q=test  # 10 requests/minute limit")
...
✅ Rate limiting now works properly - fixed initialization and middleware integration
✅ All original functionality preserved (posts, comments, files, users, admin)
✅ Proper error handling and logging throughout
✅ Development limits set low for easy testing
...
🧪 DEVELOPMENT RATE LIMITS FOR TESTING:
- CSRF Token: 3 requests per minute
- Login: 5 attempts per 5 minutes  
- Search: 10 requests per minute
- File Upload: 5 requests per 5 minutes
- Posts: 10 requests per 5 minutes
- Default: 30 requests per 5 minutes

These limits are deliberately low to make rate limiting easy to test and verify.
The system is now production-ready with all original API functionality intact.
"""
